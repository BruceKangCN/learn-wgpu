...
pub struct CameraUniform {
    // we can't use cgmath directly with bytemuck, so we'll have to cast the
...
impl CameraController {
    ...
    pub fn update_camera(&self, camera: &mut Camera) {
        ...
        if self.is_right_pressed {
            ...
            // Note: forward = forward - right because eye = target - forward.
            // The substraction of right component in forward vector makes an
            // addition to the right component of eye position.
...
impl Instance {
    pub fn to_raw_data(&self) -> InstanceRaw {
        let data =
            cgmath::Matrix4::from_translation(self.position) * cgmath::Matrix4::from(self.rotation);

        InstanceRaw { model: data.into() }
    }
}
