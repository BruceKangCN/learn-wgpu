...
use crate::{
    texture,
    util::{Camera, CameraController, CameraUniform, Instance, InstanceRaw, Vertex},
};
...
impl State {
    async fn new(window: Arc<Window>) -> Result<Self> {
        ...
        let blob = [camera_uniform]; // create a long lived `contents`
        let cbi_desc = &wgpu::util::BufferInitDescriptor {
            label: Some("Camera Buffer"),
            contents: bytemuck::cast_slice(&blob),
            usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
        };
        let camera_buffer = device.create_buffer_init(cbi_desc);

        let camera_bgl_desc = wgpu::BindGroupLayoutDescriptor {
        ...
        let camera_bgl = device.create_bind_group_layout(&camera_bgl_desc);
        ...
        let pipeline_layout_desc = wgpu::PipelineLayoutDescriptor {
            ...
            bind_group_layouts: &[Some(&texture_bgl), Some(&camera_bgl)],
            immediate_size: 0,
        };
        ...
        let instances: Vec<_> = (0..INSTANCES_PER_ROW)
            .flat_map(|z| {
            ...
            .collect();
        let instance_data: Vec<_> = instances.iter().map(Instance::to_raw_data).collect();
    ...
    fn update(&mut self) {
        ...
        let blob = [self.camera_uniform];
        let data = bytemuck::cast_slice(&blob);
